 ...
 * RowdyReviewActivity class will take input from user to add a review, refresh review, and delete review
 */
...
public class RowdyReviewActivity extends AppCompatActivity implements View.OnClickListener{

    private static final String PREFS_NAME = "RowdyReviewPrefs";
    private static final String KEY_REVIEW_COUNT = "ReviewCount";
    private static final String ROWDY_PREFIX = "rowdy_";

    private LinearLayout reviewContainer;
    private List<RowdymartReview> reviewList;
    ...
    private void displayReviews() {
        for(RowdymartReview reviews : reviewList){
            createReviewView(reviews);
        }
    }

    /**
     * load reviews from preference
     */
    private void loadReviewsFromPreferences() {
        ...
        for(int i = 0; i < reviewCount; i++){
            String title = sharedPreferences.getString(ROWDY_PREFIX + "review_title_" + i, "");
            String content = sharedPreferences.getString(ROWDY_PREFIX + "review_content_" + i, "");

            RowdymartReview review = new RowdymartReview();
     ...
     */
    private void createReviewView(final RowdymartReview review) {
    ...
    /**
     *
     * @param review ask user if they want to delete review
     */
    private void showDeleteDialog(final RowdymartReview review) {
     ...
     */
    private void deleteReviewAndRefresh(RowdymartReview review) {
    ...
    private void saveReviewsToPreferences() {
        ...
        for(int i = 0; i < reviewList.size(); i++){
            RowdymartReview review = reviewList.get(i);
            editor.putString(ROWDY_PREFIX + "review_title_" + i, review.getTitle());
            editor.putString(ROWDY_PREFIX + "review_content_" + i, review.getContent());
        }
        editor.apply();
